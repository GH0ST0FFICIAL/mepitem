...
public partial class Admin_SubSystem : System.Web.UI.Page
{
    ...
    {
        var data = db.tblSubSystems.Where(d => d.SystemId != 0).OrderByDescending(d => d.Id).ToList();
    ...
    protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Session["id"] = id;
        clearddlSystem();
        var data = db.tblSubSystems.Where(d => d.Id == id).FirstOrDefault();       
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        foreach (var item in db.tblCompanyDetails.Where(d => d.SubSystemId == data.Id).ToList())
        {
            item.SubSystemTitle = data.Title;
        }
        db.SaveChanges();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (db.tblSubSystems.Any(d => d.Title == txtTitle.Text))
        {
            Page.ClientScript.RegisterStartupScript(GetType(), "msgbox", "alert('" + txtTitle.Text + " " + "already exists!" + "');", true);
        }
        else
        {
            subsystem.Title = txtTitle.Text;
            subsystem.SystemId = Int32.Parse(ddlSystem.SelectedValue);
